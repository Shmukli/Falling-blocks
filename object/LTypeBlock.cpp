#include "LTypeBlock.h"

object::LTypeBlock::LTypeBlock(sf::Vector2f scale)
    : GameBlock{scale} {
  ...
  second_block.getBlock().setPosition(base_block.getBlock().getPosition().x,
                                      first_block.getBlock().getPosition().y + 50 * scale.y);
  ...
  this->L_block.push_back(base_block);
  this->L_block.push_back(first_block);
  this->L_block.push_back(second_block);
  this->L_block.push_back(third_block);

}

std::vector<object::GameBlock> object::LTypeBlock::getBlock() {
