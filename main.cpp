...
int main() {
    ...
    while (menuWindow.pollEvent(event)) {
      switch (event.type) {
        case sf::Event::KeyReleased:
            ...
            case sf::Keyboard::Return:
              switch (menu.GetPressedItem()) {
                case 0:std::cout << "Play button has been pressed" << std::endl;
                  ...
                  menuWindow.close();
                  break;
                case 1:std::cout << "Difficulty button has been pressed" << std::endl;
                  difficulty_coefficient -= 200;

                  break;
                case 2:menu.showHistory();
                  break;

                case 3:menu.showHelp();
                  break;

                case 4:menuWindow.close();
                  break;
              }
          ...
          break;
        case sf::Event::Closed:menuWindow.close();
  ...
  while (selected) {
    ...
    while (engine.gameWindowOpened() && !(engine.gameOver(engine.getStoredBlocks()))) {
      ...
      engine.setCollisionResult(engine.getCollision().resolveObjectCollision(engine.getActualBlocks(),
                                                                             engine.getStoredBlocks()));
